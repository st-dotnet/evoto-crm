...
>(({ className, zIndex, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ...
    className={cn(
      ...
      zIndex ? zIndex : 'z-[10000]',
      className
    )}
    style={zIndex ? { zIndex: parseInt(zIndex.replace(/\D/g, '')) } : undefined}
    {...props}
  />
));
...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & { zIndex?: string }
>(({ className, children, zIndex, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed max-h-[95%] scrollable-y-auto left-[50%] top-[50%] grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] border bg-background shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-lg',
        zIndex ? zIndex : 'z-[10000]',
        className
      )}
      style={zIndex ? { zIndex: parseInt(zIndex.replace(/\D/g, '')) } : undefined}
