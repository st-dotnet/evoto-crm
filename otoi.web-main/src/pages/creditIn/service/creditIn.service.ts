...
export const getNextCreditNoteNumber = async (): Promise<ApiResponse> => {
  ...
  try {
    ...
    return {
      success: false,
      error: error.response?.data?.error || error.response?.data?.message || 'Failed to fetch next credit note number',
      status: error?.response?.status || 500
    };
  }
};

export const createCreditNote = async (creditNoteData: CreditNoteData): Promise<ApiResponse> => {
  const token = getAuthToken();
  ...
  try {
    ...
    if (error.code === 'ERR_NETWORK') {
      ...
      errorMessage = error.response.data?.error || error.response.data?.message || 'Invalid credit note data provided';
...
export const getCreditNoteById = async (id: string): Promise<ApiResponse> => {
  ...
  try {
    ...
    const transformedData = {
      ...
      tax_total: data.charges?.tax_total || data.total_tax || data.tax_total || 0,
      discount_total: data.charges?.discount_total || data.total_discount || data.discount_total || 0,
      additional_charges_total: data.charges?.additional_charges_total || data.additional_charges || 0,
      round_off: data.charges?.round_off || data.round_off_amount || data.round_off || 0,
...
export const updateInvoiceStatus = async (invoiceId: string, status: string): Promise<ApiResponse> => {
  const token = getAuthToken();
  if (!token) {
    return {
      success: false,
  ...
  try {
    const response = await axios.put(`${API_URL}/invoices/${invoiceId}/status`, { status }, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      },
...
export const updateCreditNote = async (id: string, creditNoteData: any): Promise<ApiResponse> => {
  const token = getAuthToken();
  if (!token) {
    return {
      success: false,
  ...
  try {
    const response = await axios.put(`${API_URL}/credit-notes/${id}`, creditNoteData, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      },
...
export const getCreditNotes = async (params?: {
  ...
  try {
    ...
    if (params?.credit_note_number) queryParams.append('credit_note_number', params.credit_note_number);
    ...
    return {
      success: false,
      error: error.response?.data?.error || error.response?.data?.message || 'Failed to fetch credit notes',
      status: error?.response?.status || 500,
    };
  }
};
...
export const getCreditNoteNumbersDropdown = async (): Promise<ApiResponse> => {
  ...
  try {
    ...
    try {
      ...
      return {
        success: false,
        error: fallbackError.response?.data?.error || fallbackError.response?.data?.message || 'Failed to fetch credit note numbers',
        status: fallbackError?.response?.status || 500,
      };
    }
  }
...
export const checkCreditNoteExistsForInvoice = async (invoiceId: string): Promise<ApiResponse> => {
  const token = getAuthToken();
  if (!token) {
    return { success: false, error: 'Authentication required', status: 401 };
  }
  ...
  try {
    ...
    const linkedCreditNotes = creditNotes.filter((cn: any) => {
      ...
      return cn.linked_invoice_id === invoiceId || 
    ...
    return {
      success: false,
      error: error.response?.data?.error || error.response?.data?.message || 'Failed to check credit note status',
      status: error?.response?.status || 500,
    };
  }
};
