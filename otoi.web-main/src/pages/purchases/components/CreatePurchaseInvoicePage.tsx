...
const CreatePurchaseInvoicePage = () => {
  ...
  const handleSaveInvoice = async () => {
    ...
    try {
      ...
      if (response.success) {
        toast.success(isEditMode ? "Purchase Invoice updated!" : "Purchase Invoice created!");
        const uuid = id || response.data?.invoice_uuid || response.data?.uuid;
        navigate(`/purchases/purchase-invoices/${uuid}`);
      } else {
