...
const CreatePurchaseInvoicePage = () => {
  ...
  // Handle purchase order data from navigation state
  useEffect(() => {
    const poState = location.state as any;

    // If coming from purchase order, pre-fill form with PO data
    if (poState?.fromPurchaseOrder && poState?.purchaseOrderData) {
      const poData = poState.purchaseOrderData;

      // Set vendor
      if (poData.selectedVendor) {
        const vendorData: Vendor = {
          id: poData.selectedVendor.uuid,
          uuid: poData.selectedVendor.uuid,
          name: poData.selectedVendor.vendor_name || poData.selectedVendor.company_name || "",
          mobile: poData.selectedVendor.mobile,
          company_name: poData.selectedVendor.company_name,
          email: poData.selectedVendor.email,
          gst: poData.selectedVendor.gst,
          address1: poData.selectedVendor.address1,
          address2: poData.selectedVendor.address2,
          city: poData.selectedVendor.city,
          state: poData.selectedVendor.state,
          country: poData.selectedVendor.country,
          pin: poData.selectedVendor.pin,
        };
        setSelectedVendor(vendorData);
      }

      // Set invoice date to today (or could use PO date)
      setFormData(prev => ({
        ...prev,
        invoiceDate: today,
        dueDate: addDays(today, 30) // Default 30 days due
      }));

      // Set items from PO
      if (poData.poItems && poData.poItems.length > 0) {
        const transformedItems: InvoiceItem[] = poData.poItems.map((item: any, index: number) => ({
          id: `po-item-${index}`,
      ...
      toast.success('Invoice pre-filled from purchase order data');
    }
