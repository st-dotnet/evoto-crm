"""add ditIn table

Revision ID: 0217ed50fba2
Revises: 479ef40d9efb
Create Date: 2026-03-24 22:05:00.496825

"""
from alembic import op
import sqlalchemy as sa


# revision identifiers, used by Alembic.
revision = '0217ed50fba2'
down_revision = '479ef40d9efb'
branch_labels = None
depends_on = None


def upgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    existing_tables = inspector.get_table_names()

    if 'debit_notes' not in existing_tables:
        op.create_table('debit_notes',
        sa.Column('uuid', sa.UUID(), nullable=False),
        sa.Column('debit_note_number', sa.String(length=50), nullable=False),
        sa.Column('vendor_id', sa.UUID(), nullable=False),
        sa.Column('business_id', sa.Integer(), nullable=False),
        sa.Column('invoice_id', sa.UUID(), nullable=True),
        sa.Column('invoice_number', sa.String(length=50), nullable=True),
        sa.Column('original_invoice_payment_status', sa.String(length=20), nullable=True),
        sa.Column('debit_note_date', sa.Date(), nullable=False),
        sa.Column('status', sa.String(length=20), nullable=False),
        sa.Column('total_amount', sa.Numeric(precision=12, scale=2), nullable=False),
        sa.Column('amount_received', sa.Numeric(precision=12, scale=2), nullable=True),
        sa.Column('balance_amount', sa.Numeric(precision=12, scale=2), nullable=True),
        sa.Column('charges', sa.JSON(), nullable=True),
        sa.Column('mark_as_fully_paid', sa.Boolean(), nullable=True),
        sa.Column('auto_round_off', sa.Boolean(), nullable=True),
        sa.Column('is_deleted', sa.Boolean(), nullable=False),
        sa.Column('additional_notes', sa.JSON(), nullable=True),
        sa.Column('created_by', sa.UUID(), nullable=True),
        sa.Column('updated_by', sa.UUID(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(['business_id'], ['businesses.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['created_by'], ['users.uuid'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['updated_by'], ['users.uuid'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['vendor_id'], ['vendors.uuid'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('uuid'),
        sa.UniqueConstraint('debit_note_number')
        )
    
    if 'debit_note_items' not in existing_tables:
        op.create_table('debit_note_items',
        sa.Column('uuid', sa.UUID(), nullable=False),
        sa.Column('debit_note_id', sa.UUID(), nullable=False),
        sa.Column('item_id', sa.UUID(), nullable=True),
        sa.Column('hsn_sac_code', sa.String(length=20), nullable=True),
        sa.Column('description', sa.Text(), nullable=True),
        sa.Column('quantity', sa.Numeric(precision=10, scale=2), nullable=False),
        sa.Column('unit_price', sa.Numeric(precision=12, scale=2), nullable=False),
        sa.Column('discount', sa.JSON(), nullable=True),
        sa.Column('tax', sa.JSON(), nullable=True),
        sa.Column('total_price', sa.Numeric(precision=12, scale=2), nullable=False),
        sa.Column('created_by', sa.UUID(), nullable=True),
        sa.Column('updated_by', sa.UUID(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(['created_by'], ['users.uuid'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['debit_note_id'], ['debit_notes.uuid'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['item_id'], ['items.id'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['updated_by'], ['users.uuid'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('uuid')
        )

    if 'debit_note_payments' not in existing_tables:
        op.create_table('debit_note_payments',
        sa.Column('uuid', sa.UUID(), nullable=False),
        sa.Column('debit_note_id', sa.UUID(), nullable=False),
        sa.Column('invoice_id', sa.UUID(), nullable=True),
        sa.Column('payment_amount', sa.Numeric(precision=12, scale=2), nullable=False),
        sa.Column('payment_date', sa.Date(), nullable=False),
        sa.Column('payment_method', sa.String(length=20), nullable=False),
        sa.Column('payment_reference', sa.String(length=100), nullable=True),
        sa.Column('payment_notes', sa.Text(), nullable=True),
        sa.Column('status', sa.String(length=20), nullable=False),
        sa.Column('is_deleted', sa.Boolean(), nullable=False),
        sa.Column('created_by', sa.UUID(), nullable=True),
        sa.Column('updated_by', sa.UUID(), nullable=True),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('updated_at', sa.DateTime(), nullable=True),
        sa.ForeignKeyConstraint(['created_by'], ['users.uuid'], ondelete='SET NULL'),
        sa.ForeignKeyConstraint(['debit_note_id'], ['debit_notes.uuid'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['invoice_id'], ['invoices.uuid'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['updated_by'], ['users.uuid'], ondelete='SET NULL'),
        sa.PrimaryKeyConstraint('uuid')
        )

    with op.batch_alter_table('invoices', schema=None) as batch_op:
        # We can't easily check for nullability without more logic, 
        # but alter_column is generally safer than create_table.
        batch_op.alter_column('status',
               existing_type=sa.VARCHAR(length=20),
               nullable=False)

    # ### end Alembic commands ###



def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table('invoices', schema=None) as batch_op:
        batch_op.alter_column('status',
               existing_type=sa.VARCHAR(length=20),
               nullable=True)

    op.drop_table('debit_note_payments')
    op.drop_table('debit_note_items')
    op.drop_table('debit_notes')
    # ### end Alembic commands ###
