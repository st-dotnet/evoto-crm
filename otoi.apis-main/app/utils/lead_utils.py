...
#     if hasattr(lead.status, 'lower'):
#         status_str = str(lead.status).strip().lower()
#     else:
#         status_int = int(lead.status) if lead.status is not None else None
#         if status_int != 4:
#             return None
#         status_str = "4"  # Convert to string for consistency
...
def sync_lead_to_customer(lead, address_data=None):
    ...
    if address_data:
        ...
        # Also update the Address record to link it to this customer
        lead_address = LeadAddress.query.filter_by(lead_id=lead.uuid).first()
        if lead_address:
            address = lead_address.address
            if address:
                address.customer_id = customer.uuid
                set_updated_fields(address)
    else:
        # If no address_data passed, try to fetch from lead's existing address
        # but only if we don't have better data.
        # For now, let's just ensure NOT NULL columns have something.
