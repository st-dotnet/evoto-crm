...
def create_customer():
    ...
    # Shipping Address Data - Handle new shipping_addresses array format
    shipping_addresses = data.get("shipping_addresses", [])

    # Validate shipping addresses array
    if shipping_addresses:
        if not isinstance(shipping_addresses, list):
            return jsonify({"error": "shipping_addresses must be an array"}), 400

        # Validate each shipping address
        for i, addr in enumerate(shipping_addresses):
            if not isinstance(addr, dict):
                return (
                    jsonify(
                        {"error": f"Shipping address at index {i} must be an object"}
                    ),
                    400,
                )
            ...
            if missing_fields:
                return (
                    jsonify(
                        {
                            "error": f"Shipping address at index {i} missing required fields: {', '.join(missing_fields)}"
                        }
                    ),
                    400,
                )
            ...
            if address_type not in ["home", "work", "other"]:
                return (
                    jsonify(
                        {
                            "error": f"Shipping address at index {i} has invalid address_type. Must be 'home', 'work', or 'other'"
                        }
                    ),
                    400,
                )
            ...
            addr["is_default"] = bool(addr.get("is_default", False))

    # Check for maximum of 3 shipping addresses
    if len(shipping_addresses) > 3:
        return (
            jsonify({"error": "Maximum of 3 shipping addresses allowed per customer"}),
            400,
        )
    ...
    if mobile:
        query = Customer.query.filter(
            Customer.mobile == mobile, Customer.is_deleted == False
        )
        if uuid_to_ignore:
    ...
    if gst and gst.strip():  # Check if GST is provided and not empty
        ...
        gst_query = Customer.query.filter(
            func.upper(Customer.gst) == gst, Customer.is_deleted == False
        )
        if uuid_to_ignore:
        ...
        if mobile or email:
            ...
            if mobile:
                lead = Lead.query.filter(
                    Lead.mobile == mobile, Lead.is_deleted == False
                ).first()
            if not lead and email:
                lead = Lead.query.filter(
                    Lead.email == email, Lead.is_deleted == False
                ).first()
    ...
    if existing_customer:
        ...
        # Handle shipping addresses array for updates
        if shipping_addresses:
            try:
                current_app.logger.info(
                    f"Processing {len(shipping_addresses)} shipping addresses for existing customer {existing_customer.uuid}"
                )

                # First, unset all existing defaults if any new address is set as default
                has_default = any(
                    addr.get("is_default", False) for addr in shipping_addresses
                )
                if has_default:
                ...
                for addr_data in shipping_addresses:
                    ...
                    if existing_address:
                        ...
                        for field in ["address1", "city", "state", "country", "pin"]:
                            setattr(existing_address, field, addr_data[field].strip())

                        existing_address.is_default = addr_data["is_default"]
                        set_updated_fields(existing_address)
                        current_app.logger.info(
                            f"Updated existing shipping address type: {addr_data['address_type']}"
                        )
                        ...
                        shipping = Shipping(
                            customer_id=existing_customer.uuid,
                            address1=addr_data["address1"].strip(),
                            city=addr_data["city"].strip(),
                            state=addr_data["state"].strip(),
                            country=addr_data["country"].strip(),
                            pin=addr_data["pin"].strip(),
                            address_type=addr_data["address_type"],
                        ...
                        current_app.logger.info(
                            f"Created new shipping address type: {addr_data['address_type']}"
                        )

                db.session.flush()
                current_app.logger.info("Processed shipping addresses successfully")

            except Exception as e:
                db.session.rollback()
                current_app.logger.error(
                    f"Error processing shipping addresses: {str(e)}", exc_info=True
                )
                return (
                    jsonify(
                        {"error": f"Failed to process shipping addresses: {str(e)}"}
                    ),
                    500,
                )
        ...
        # Get all shipping addresses for response
        updated_shipping_addresses = (
            Shipping.query.filter(Shipping.customer_id == existing_customer.uuid)
            .order_by(Shipping.is_default.desc())  # Put default address first
            .all()
        )
            ...
            response_data["shipping_addresses"] = [
                {
                    ...
                    "created_at": (
                        addr.created_at.isoformat() if addr.created_at else None
                    ),
                    "updated_at": (
                        addr.updated_at.isoformat() if addr.updated_at else None
                    ),
                }
                for addr in updated_shipping_addresses
            ]
    ...
    # Handle shipping addresses array
    if shipping_addresses:
        try:
            current_app.logger.info(
                f"Processing {len(shipping_addresses)} shipping addresses for customer {customer.uuid}"
            )
            ...
            if has_default:
                Shipping.query.filter(Shipping.customer_id == customer.uuid).update(
            ...
            for addr_data in shipping_addresses:
                # Check if this address type already exists for this customer
                existing_address = Shipping.query.filter_by(
                    customer_id=customer.uuid, address_type=addr_data["address_type"]
                ).first()
            ...
            return (
                jsonify({"error": f"Failed to process shipping addresses: {str(e)}"}),
                500,
            )
    ...
    # Get all shipping addresses for response
    shipping_addresses = (
        Shipping.query.filter(Shipping.customer_id == customer.uuid)
        .order_by(Shipping.is_default.desc())  # Put default address first
        .all()
    )
...
def update_customer(customer_id):
    ...
    try:
        ...
        has_shipping_updates = "shipping_addresses" in data or any(
            k in data
            for k in [
                "shipping_address1",
                "shipping_city",
        ...
        if has_shipping_updates:
            ...
            if "shipping_addresses" in data:
                ...
                new_addresses = [
                    ...
                    if "uuid" not in addr or addr["uuid"] not in existing_uuids
                ]
                if new_addresses and existing_shipping_count + len(new_addresses) > 3:
                    ...
                    if to_remove > 0:
                        ...
                        oldest_shippings = (
                            Shipping.query.filter_by(customer_id=customer.uuid)
                            .order_by(Shipping.created_at.asc())
                            .limit(to_remove)
                            .all()
                        )
            ...
            def is_valid_shipping_payload(ship):
                ...
                missing_fields = [
                    ...
                    if not value and field != "address2"
                ]
                ...
                if required_fields["address_type"] not in ["home", "work", "other"]:
                    current_app.logger.error(
                        f"Invalid address_type: {required_fields['address_type']}"
                    )
                    return False
                return True
        ...
        if "shipping_addresses" in data and isinstance(
            data["shipping_addresses"], list
        ):
            try:
                # Get existing shipping count and list
                existing_shipping = Shipping.query.filter_by(
                ...
                existing_count = len(existing_shipping)
                incoming_count = len(data["shipping_addresses"])
                # Check address limit - existing + incoming
                if existing_count + incoming_count > 3:
                    # Remove oldest addresses if needed
                    to_remove = (existing_count + incoming_count) - 3
                    if to_remove > 0:
                        oldest_shippings = (
                            Shipping.query.filter_by(customer_id=customer.uuid)
                            .order_by(Shipping.created_at.asc())
                ...
                existing_shipping = {
                    str(addr.uuid): addr
                    for addr in Shipping.query.filter_by(
                        customer_id=customer.uuid
                    ).all()
                }
                ...
                for ship_data in data["shipping_addresses"]:
                    ...
                    if missing_fields:
                        ...
                        return (
                            jsonify(
                                {
                                    "error": f"Missing required shipping address fields: {', '.join(missing_fields)}"
                                }
                            ),
                            400,
                        )
                    ...
                    # Always allow default switch - unset all existing defaults first
                ...
                return (
                    jsonify({"error": f"Failed to process shipping address: {str(e)}"}),
                    400,
                )
        ...
        if shipping_list:
            ...
            has_default = Shipping.query.filter_by(
                customer_id=customer.uuid, is_default=True
            ).first()

            if (
                not has_default and shipping_list
            ):  # If no default but we have addresses, set the first one as default
        ...
        try:
            ...
            if cleaned_count > 0:
                current_app.logger.info(
                    f"Cleaned up {cleaned_count} orphaned addresses"
                )
            db.session.commit()
            ...
            return (
                jsonify(
                    {"error": "Failed to update customer due to an internal error"}
                ),
                500,
            )
        # ---------------- RESPONSE ----------------
            ...
            response.update(
                {
                    ...
                    "address_type": default_shipping.address_type,
                    "is_default_shipping": default_shipping.is_default,
                    "shipping_uuid": str(default_shipping.uuid),
                }
            )

        # Get all shipping addresses for the response
        ...
        if all_shipping_addresses:
            response["shipping_addresses"] = [
                ...
                for addr in all_shipping_addresses
            ]
