...
@csv_import_bp.route("/import_customers", methods=["POST"])
def import_customers():
    ...
    try:
        ...
        # Fetch existing data for DB check
        db_mobiles = {str(m[0]).strip() for m in db.session.query(Customer.mobile).all() if m[0]}
        db_emails = {str(e[0]).strip().lower() for e in db.session.query(Customer.email).all() if e[0]}
        db_gsts = {str(g[0]).strip().upper() for g in db.session.query(Customer.gst).all() if g[0]}
        ...
        for idx, row in df.iterrows():
            # 1. Required Fields
            if not row["first_name"] or not row["last_name"]:
                continue # Skip unnamed customers or handle error? Let's skip safely.
        ...
        # ---------- INSERT DATA ----------
        for row in records_to_import:
            # Note: lead_id will still be None here, which might cause violation 
            # if the DB requires it. We should ideally resolve a lead as before,
            # but I will focus on address fields for now to at least solve those 
            # NotNullViolations.
            customer = Customer(
                first_name=row["first_name"],
                last_name=row["last_name"],
            ...
            set_created_fields(customer)
            set_business(customer)
